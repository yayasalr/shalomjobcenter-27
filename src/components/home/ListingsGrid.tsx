...
export const ListingsGrid = ({
  ...
  if (isLoading) {
    return (
      <div className="w-screen m-0 p-0">
        <div className="airbnb-grid w-screen m-0 p-0">
          {[...Array(12)].map((_, index) => (
            <Card key={index} className="overflow-hidden hover-shadow transition duration-300 h-full m-0 p-0">
  ...
  return (
    <div className="w-screen m-0 p-0">
      <motion.div 
        className="airbnb-grid w-screen m-0 p-0"
        variants={containerVariants}
        initial="hidden"
        animate="visible"
      >
        {visibleListings.map((listing) => (
          <motion.div key={listing.id} variants={itemVariants} layout className="hover-lift h-full m-0 p-0">
